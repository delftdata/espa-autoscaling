...
public class TestBidPersonGeneratorKafka {
    ...
    private void getTimestampUsforEventTest(long epochStartTimeMs, long epochDurationMs, long eventsPerEpoch,
        ...
        long actualTimestamp =this.timestampUsTestGeneratorFunction.getTimestampUsforEvent(epochStartTimeMs,
    ...
    public void testGenerationUniqueTimestampsIncreasingInputRate() {
        ...
        for (int epoch = 0; epoch < 100; epoch++) {
            ...
            generatorFunction.setNextEpochSettings(epochInputRate);
            for (int i = 0; i < epochInputRate; i++ ) {
                long eventNumber = generatorFunction.getNextEventId();
                eventNumbers.add(eventNumber);
                long timestamp = generatorFunction.getTimestampUsforEvent(eventNumber);
