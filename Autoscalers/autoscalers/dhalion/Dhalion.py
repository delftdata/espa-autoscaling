...
from common.Autoscaler import Autoscaler
from dhalion.DhalionMetricsGatherer import DhalionMetricsGatherer
...
class Dhalion(Autoscaler, ABC):
    ...
    def runDhalionIteration(self):
        ...
        time.sleep(self.configurations.DHALION_MONITORING_PERIOD_SECONDS)
        ...
        if self.metricsGatherer.isSystemBackpressured(backpressureStatusMetrics=backpressureStatusMetrics):
            ...
            backpressureTimeMetrics: {str, float} = self.metricsGatherer.gatherBackpressureTimeMetrics(
                monitoringPeriodSeconds=self.configurations.DHALION_MONITORING_PERIOD_SECONDS)
    ...
    def run(self):
        ...
        :return: None
        """
        print("Running Dhalion autoscaler.")
        while True:
            try:
                self.runDhalionIteration()
            except:
                traceback.print_exc()
