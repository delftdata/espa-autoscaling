...
AUTOSCALER=${5}
AUTOSCALER_CONFIGURATION_NAME=${6}
...
EXPERIMENT_IDENTIFIER="${EXPERIMENT_TAG}q${QUERY}${LOAD_PATTERN}_${AUTOSCALER}(${AUTOSCALER_CONFIGURATION_NAME})${MODE}"
echo "Fetching data for ${EXPERIMENT_IDENTIFIER} with duration ${EXPERIMENT_DURATION}"
...
python3 ../data_fetching "${PROMETHEUS_IP}" "${PROMETHEUS_PORT}" "${EXPERIMENT_DURATION}" "${DATA_STEP_SIZE_SECONDS}" "${DATA_DIRECTORY}" "${EXPERIMENT_IDENTIFIER}"
