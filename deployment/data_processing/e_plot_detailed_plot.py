...
def plot_detailed_plot(query, auto_scaler, percentage, load_pattern="cosine-60", metrics=detailed_metrics):
    ...
    meric_names = {"input_rate": "Kafka input rate (records per second)", "taskmanager": "Taskmanagers",
                   "latency": "Latency (s)", "throughput": "Throughput (records per second)",
                   "backpressure": "Backpressure (ms)", "CPU_load": "CPU utilization", "lag": "Lag (records)",
                   "busy_time": "Busy time (ms)", "idle_time": "Idle time (ms)"}
    ylabel = {"input_rate": "records per seconds", "taskmanager": "# taskmanagers", "latency": "seconds",
              "CPU_load": "", "backpressure": "ms"}
