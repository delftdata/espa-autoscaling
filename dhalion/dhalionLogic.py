...
while True:
        ...
        scaling_factor = int(os.environ['SCALING_FACTOR'])
        ...
        if float(previous_scaling_event) == 0:
                ...
                if (float(backpressure_value) > backpressure_upper_threshold or float(cpu_upper_threshold)) and current_number_of_taskmanagers < max_replicas:
                                if new_number_of_taskmanagers + scaling_factor <= max_replicas:
                                        new_number_of_taskmanagers += scaling_factor
                                else:
                                ...
                                if new_number_of_taskmanagers - scaling_factor >= min_replicas:
                                        new_number_of_taskmanagers -= scaling_factor
                                else:
