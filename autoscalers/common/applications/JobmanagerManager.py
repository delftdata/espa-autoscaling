...
class JobmanagerManager:
    ...
    def getOperatorHostInformation(self, job_id=None, job=None) -> [str, [(str, str, int)]]:
        """
        ...
        for verticeJSON in verticeJSONs:
            operatorName = self.configurations.experimentData \
                .convert_operator_name_to_prometheus_operator_name(verticeJSON['name'])
    ...
    def getOperators(self, jobPlan=None) -> [str]:
        """
        ...
        nodes = list(map(lambda node: self.configurations.experimentData.
                         convert_operator_name_to_prometheus_operator_name(node['description']), node_json))

        # TODO: Check whether this works. Originally it called self.configurations.experimentData
        print(f"TODO: CHECK WHETHER getOperators works: {node_json}, {nodes}")
        return nodes

    def getIdOperatorMapping(self, jobPlan=None) -> dict[str, str]:
        """
        ...
        for n in node_json:
            operator_name = self.configurations.experimentData \
                .convert_operator_name_to_prometheus_operator_name(n['description'])
            operator_id = n['id']
            operator_id_mapping[operator_id] = operator_name
        return operator_id_mapping

    def getOperatorParallelism(self, jobPlan=None) -> dict[str, int]:
        """
        ...
        for node in nodes:
            nodeName = self.configurations.experimentData \
                .convert_operator_name_to_prometheus_operator_name(node['description'])
    ...
    def getTopology(self, jobPlan=None) -> [(str, str)]:
        ...
        for node_json in jobPlan['plan']['nodes']:
            node_name = self.configurations.experimentData \
                .convert_operator_name_to_prometheus_operator_name(node_json['description'])
                ...
                for edge_in_json in node_json['inputs']:
                    ...
                    if edge_in_operator_id in id_operator_mapping.keys():
                        node_in_name = self.configurations.experimentData \
                            .convert_operator_name_to_prometheus_operator_name(id_operator_mapping[edge_in_operator_id])
                        topology.append((node_in_name, node_name))
        return topology

    # Trigger savepoint and get the required information
    def triggerSavepointAndGetTriggerId(self, job_id=None) -> int:
        """
    ...
    def getSavePointTriggerJSON(self, job_id, trigger_id) -> Any:
        """
    ...
    def sendStopJobRequest(self, job_id=None) -> Any:
        """
    ...
    def sendStopJobRequestAndGetSavePointTriggerId(self, job_id=None) -> (Any, str):
        """
    ...
    def extractSavePointStatusFromSavePointTriggerJSON(self, job_id, trigger_id) -> (str, str):
        """
    ...
    def getReadyUnreadyTaskmanagers(self) -> ([str], [str]):
        """
        Get a tuple of two list ([ready taskmanagers], [unready taskmanagers]) from the current jobmanager.
        :return: a tuple of two list ([ready taskmanagers], [unready taskmanagers])
        """
